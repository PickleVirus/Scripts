...
/****** Object:  Job [Back Up Database SES, DiscountShop and TerminalSAP: Tue Fri]    Script Date: 12/11/2015 15:05:08 ******/
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'Back Up Database SES, DiscountShop and TerminalSAP: Tue Fri')
EXEC msdb.dbo.sp_delete_job @job_id=N'f5dbd229-a572-4105-9f87-5800f37db060', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'copy ses to backup_server', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'xcopy E:\dbbackup\SES.7z \\10.81.32.4\E$\bkpsrv\dbbackup_2 /Y', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'copy discountshop to backup_server', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'xcopy E:\dbbackup\DiscountShop.7z \\10.81.32.4\E$\bkpsrv\dbbackup_2 /Y', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'copy terminalsap to backup_server', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'xcopy E:\dbbackup\TerminalSAP.7z \\10.81.32.4\E$\bkpsrv\dbbackup_2 /Y',
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'copy SES_DC to backup_server', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'xcopy E:\dbbackup\SES_DC.7z \\10.81.32.4\E$\bkpsrv\dbbackup_2 /Y',
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
